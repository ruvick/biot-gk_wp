<?php
/**
 * biot functions and definitions
 *
...
if ( ! function_exists( 'biot_setup' ) ) :
	...
	function biot_setup() {
		 ...
		 * If you're building a theme based on biot, use a find and replace
		 * to change 'biot' to the name of your theme in all the template files.
		 */
		load_theme_textdomain( 'biot', get_template_directory() . '/languages' );
		...
		register_nav_menus(
			array(
				'menu-1' => esc_html__( 'Primary', 'biot' ),
			)
		);
...
add_action( 'after_setup_theme', 'biot_setup' );
 ...
 */
function biot_content_width() {
	// This variable is intended to be overruled from themes.
	// Open WPCS issue: {@link https://github.com/WordPress-Coding-Standards/WordPress-Coding-Standards/issues/1043}.
	// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedVariableFound
	$GLOBALS['content_width'] = apply_filters( 'biot_content_width', 640 );
}
add_action( 'after_setup_theme', 'biot_content_width', 0 );
 ...
 */
function biot_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'biot' ),
			'id'            => 'sidebar-1',
			'description'   => esc_html__( 'Add widgets here.', 'biot' ),
...
add_action( 'widgets_init', 'biot_widgets_init' );

/**
 * Enqueue scripts and styles.
 */
function biot_scripts() {
	...
	wp_enqueue_style( 'biot-style', get_stylesheet_uri(), array(), '1.2.1' );
	wp_style_add_data( 'biot-style', 'rtl', 'replace' );

	wp_enqueue_script('jquery');
	wp_enqueue_script( 'biot-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
...
add_action( 'wp_enqueue_scripts', 'biot_scripts' );
...
add_action( 'wp_ajax_universal_send', 'universal_send' );
  ...
  function universal_send() {
    ...
    if ( check_ajax_referer( 'NEHERTUTLAZIT', 'nonce', false ) ) {
      ...
      $headers = array(
        'From: Сайт Biot-gk <noreply@biot-gk.ru>',
